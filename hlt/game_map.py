...
class Map:
    ...
    def priorized_conquerer(self, ship):

        result = {}
        for foreign_entity in self.all_planets():
            if entity == foreign_entity or foreign_entity.is_owned() or foreign_entity in ignore:
                continue
            result.setdefault(entity.calculate_distance_between(
    ...
    def nearest_enemy_ship_by_distance(self, entity, ignore_list=[]):
        """
        :param entity: The source entity to find distances from
        :return: Dict containing all entities with their designated distances
        :rtype: dict
        """
        result = {}
        my_ships = self.get_me().all_ships()
        for foreign_entity in self._all_ships():
            if foreign_entity in my_ships or foreign_entity == None or foreign_entity.id in ignore_list:
                continue
            result.setdefault(entity.calculate_distance_between(
                foreign_entity), []).append(foreign_entity)
        return result

    def nearby_planet_by_distance(self, entity):
        """
        :param entity: The source entity to find distances from
        :return: Dict containing all entities with their designated distances
        :rtype: dict
    ...
    def nearby_unowned_planet_by_distance(self, entity, ignore=[]):
        """
        :param entity: The source entity to find distances from
        :return: Dict containing all entities with their designated distances
        :rtype: dict
    ...
    def nearby_owned_planet_by_distance(self, entity, ignore=[]):
        result = {}
        for foreign_entity in self.all_planets():
            if entity == foreign_entity or not foreign_entity.is_owned()or foreign_entity in ignore:
                continue
            result.setdefault(entity.calculate_distance_between(
                foreign_entity), []).append(foreign_entity)
        return result

    def nearby_unsecure_owned_planet_by_distance(self, entity, ignore=[]):
        result = {}
        for foreign_entity in self.all_planets():
            if entity == foreign_entity or not foreign_entity.is_owned() or foreign_entity.is_full() or foreign_entity in ignore:
                continue
            result.setdefault(entity.calculate_distance_between(
                foreign_entity), []).append(foreign_entity)
        return result

    def nearby_enemy_planet(self, entity, ignore=[]):
        planets = set()
        ...
        #### error in this
        result = {}
        for foreign_entity in planets:
            if foreign_entity in ignore:
                continue
    ...
    def nearby_owned_planet_by_us_not_full(self, entity):
        result = {}
        ships = all_docked_ships()  # get our ships

        # is planet is owned but not full priorities
        for foreign_entity in self.all_planets():
            if entity == foreign_entity or not foreign_entity.is_owned():
                continue
            result.setdefault(entity.calculate_distance_between(
                foreign_entity), []).append(foreign_entity)
        return result
    ...
    def get_nearest_unowned_planet(self, ship):
        lowest_distance = 999
        lowest_planet = None
        for foreign_entity in self.all_planets():
            if entity == foreign_entity or foreign_entity.is_owned():
                continue
            current_distance = entity.calculate_distance_between(foreign_entity)
            if current_distance < lowest_distance:
                lowest_distance = current_distance
                lowest_planet = foreign_entity
        return lowest_planet
